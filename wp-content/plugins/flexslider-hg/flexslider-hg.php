...
function show_flexslider_rotator( $slug )
{
	...
	if ( have_posts() ) 
	{
		...
		while ( have_posts() )
		{
			...
			$a_tag_opening2 = '<a href="' . $url . '" title="' . the_title_attribute( array('echo' => false) ) . '" ' . $targetblank . 'style="' . 'background: url(' . "'" . $url2 . "') no-repeat center center" . '"' . 'class="' . 'a-slide' . '"' . ' >';
